...
apiClient.interceptors.request.use(async (config) => {
  ...
  if (typeof window !== 'undefined') {
    // Harvis bridge: served same-origin under /onb inside Harvis, this app shares
    // Harvis's OWUI JWT at localStorage.token. Prefer it (the onb_compat facade
    // validates it via Harvis get_current_user); fall back to open-notebook's own
    // auth-storage when running standalone.
    let harvisToken: string | null = null
    try {
      harvisToken = localStorage.getItem('token')
    } catch (e) {
      harvisToken = null
    }
    if (harvisToken) {
      config.headers.Authorization = `Bearer ${harvisToken}`
    } else {
