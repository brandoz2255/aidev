...
export const notebooksApi = {
  ...
  // AI-generate a broad-scope title + emoji + synopsis (persisted to description) for a
  // notebook from its sources.
  autoname: async (id: string) => {
    const response = await apiClient.post<{ title: string; emoji?: string | null; description?: string | null }>(
      `/notebooks/${id}/autoname`
    )
    return response.data
  },
