...
import type { CitationMeta } from '@/lib/types/api'
...
export function parseSourceReferences(text: string): ParsedReference[] {
  ...
  // id allows dashes so full UUID source ids (e.g. 8eb90fd0-fc9f-…) parse intact.
...
export function convertReferencesToCompactMarkdown(text: string, referencesLabel: string = 'References'): string {
  ...
  // NotebookLM-style: no raw "References: [1] source:<uuid>" footer — the inline
  // numbered chips carry a hover preview + click-to-open, and the Sources panel
  // already lists every source. (referencesLabel is kept for signature compat.)
  void referencesLabel
  return result
}
...
export function createCompactReferenceLinkComponent(
  ...
  citationsById?: Record<string, CitationMeta>
) {
  const CompactReferenceLinkComponent = ({
    ...
    if (href?.startsWith('#ref-')) {
      ...
      const citationButton = (
        <button
          onClick={(e) => {
            e.preventDefault()
            e.stopPropagation()
            onReferenceClick(type, id, meta?.locator)
          }}
          className="inline-flex items-center justify-center align-super mx-[1px] h-[1.05rem] min-w-[1.05rem] rounded-full bg-primary/15 px-1 text-[0.65rem] font-semibold leading-none text-primary no-underline transition-colors hover:bg-primary/30 cursor-pointer"
          type="button"
        >
          {children}
        </button>
      ...
      // No metadata → plain clickable number (still opens the source).
      if (!meta) return citationButton

      // NotebookLM-style hover preview: source title + relevant snippet + View source.
      return (
        <Tooltip>
          ...
          <TooltipContent
            ...
            align="start"
            sideOffset={4}
            className="max-w-xs space-y-1.5 p-3 text-left bg-popover text-popover-foreground border border-border shadow-md"
          >
            {meta.title && (
              <div className="text-xs font-semibold leading-snug">{meta.title}</div>
            )}
            {meta.snippet && (
              <div className="text-xs text-muted-foreground leading-snug line-clamp-4">
                {meta.snippet}
              </div>
            )}
            <button
...
// ─── Per-occurrence citations ([cite:N]) — NotebookLM-style ────────────────────
// Backend emits one [cite:N] per cited chunk (N = chunk index). Each carries its
// OWN locator so a chip jumps to the exact passage. We assign DISPLAY numbers by
// distinct source (stable across the answer), dedup adjacent same-source markers
// into one chip, and group different sources as adjacent chips. Legacy [source:uuid]
// markers (older messages) are also handled, keyed by source id.

const CITE_CHIP_CLASS =
  'inline-flex items-center justify-center align-super mx-[1px] h-[1.05rem] min-w-[1.05rem] rounded-full bg-primary/15 px-1 text-[0.65rem] font-semibold leading-none text-primary no-underline transition-colors hover:bg-primary/30 cursor-pointer'

const CITE_MARKER = String.raw`\[(?:cite:\d+|source:[a-zA-Z0-9_-]+)\]`
const CITE_SINGLE_RE = /\[(?:cite:(\d+)|source:([a-zA-Z0-9_-]+))\]/

function _citeMaps(citations: CitationMeta[]) {
  const byIndex = new Map<number, CitationMeta>()
  const bySource = new Map<string, CitationMeta>()
  for (const c of citations || []) {
    if (typeof c.index === 'number') byIndex.set(c.index, c)
    if (c.source_id && !bySource.has(c.source_id)) bySource.set(c.source_id, c)
  }
  return { byIndex, bySource }
}
...
export interface CiteOccurrence {
  ...
  snippet: string
  locator: string
  /** The text of the sentence/quote in the answer that this citation supports.
   * Used to highlight the EXACT supporting passage in the source (the chunk
   * `locator` is only a fallback — it points at the chunk's start, which is often
   * a different sentence than the one cited). */
  claim: string
}

// Extract the claim a citation supports: the text on the same line/paragraph just
// before the marker (the model usually states or quotes the supported fact right
// before [cite:N]). The matcher front-trims framing words + ignores quotes, so a
// generous window is fine and robust.
function _extractClaim(text: string, runStart: number): string {
  let before = text.slice(Math.max(0, runStart - 280), runStart)
  before = before.replace(new RegExp(CITE_MARKER, 'g'), ' ')
  const nl = before.lastIndexOf('\n')
  if (nl >= 0 && before.length - nl > 24) before = before.slice(nl + 1)
  return before.replace(/\s+/g, ' ').trim()
}

/**
 * Rewrite [cite:N] / [source:uuid] markers into compact numbered chips, numbered by
 * DISTINCT SOURCE, deduped within an adjacent run, grouped as adjacent chips. Returns
 * the rewritten markdown plus an `occurrences` array — each chip's href is
 * `#cref-<occIdx>` so the link component can resolve its source + locator + the
 * claim text it supports (for exact-passage highlighting).
 */
export function convertCiteMarkers(
  text: string,
  citations: CitationMeta[]
): { markdown: string; occurrences: CiteOccurrence[] } {
  const occurrences: CiteOccurrence[] = []
  if (!text) return { markdown: text, occurrences }
  const { byIndex, bySource } = _citeMaps(citations)

  const resolve = (marker: string): CitationMeta | null => {
    const m = marker.match(CITE_SINGLE_RE)
    if (!m) return null
    if (m[1]) return byIndex.get(parseInt(m[1], 10)) || null
    return bySource.get(m[2]) || { source_id: m[2], title: '', snippet: '', locator: '' }
  }

  // Display number per distinct source, first appearance in text order.
  const dispBySource = new Map<string, number>()
  let nextDisp = 1
  for (const mm of text.matchAll(new RegExp(CITE_MARKER, 'g'))) {
    const sid = resolve(mm[0])?.source_id || ''
    if (sid && !dispBySource.has(sid)) dispBySource.set(sid, nextDisp++)
  }

  const runRe = new RegExp(`${CITE_MARKER}(?:\\s*,?\\s*${CITE_MARKER})*`, 'g')
  const markdown = text.replace(runRe, (run: string, offset: number) => {
    const claim = _extractClaim(text, offset)
    const markers = run.match(new RegExp(CITE_MARKER, 'g')) || []
    const seen = new Map<number, number>() // display number → occurrence index
    for (const mk of markers) {
      const cit = resolve(mk)
      const sid = cit?.source_id || ''
      if (!sid) continue
      const disp = dispBySource.get(sid)
      if (!disp || seen.has(disp)) continue
      seen.set(disp, occurrences.length)
      occurrences.push({
        sourceId: sid,
        title: cit?.title || '',
        snippet: cit?.snippet || '',
    ...
    return [...seen.entries()]
      .sort((a, b) => a[0] - b[0])
      .map(([disp, occIdx]) => `[${disp}](#cref-${occIdx})`)
      .join('')
  })
...
/**
 * ReactMarkdown <a> component for `#cref-<occIdx>` chip links: renders a NotebookLM-
 * style numbered chip + hover preview, and on click opens the source and highlights
 * the EXACT cited passage (claim text, with the chunk locator as fallback).
 */
export function createCiteLinkComponent(
  onReferenceClick: (type: ReferenceType, id: string, locator?: string, claim?: string) => void,
  occurrences: CiteOccurrence[]
) {
  const CiteLinkComponent = ({
    href,
    children,
    ...props
  }: React.AnchorHTMLAttributes<HTMLAnchorElement> & {
    ...
    const occ = href?.startsWith('#cref-') ? occurrences[parseInt(href.slice(6), 10)] : undefined
    if (!occ) {
      return (
        <a href={href} target="_blank" rel="noopener noreferrer" {...props} className="text-primary hover:underline">
          {children}
    ...
    const open = () => onReferenceClick('source', occ.sourceId, occ.locator, occ.claim)
    const chip = (
      <button
        type="button"
    ...
    return (
      <Tooltip>
        ...
        <TooltipContent
          ...
          className="max-w-xs space-y-1.5 p-3 text-left bg-popover text-popover-foreground border border-border shadow-md"
        >
          {occ.title && <div className="text-xs font-semibold leading-snug">{occ.title}</div>}
          {occ.snippet && (
            <div className="text-xs text-muted-foreground leading-snug line-clamp-4">{occ.snippet}</div>
          )}
          <button
            type="button"
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); open() }}
  ...
  CiteLinkComponent.displayName = 'CiteLinkComponent'
  return CiteLinkComponent
}
