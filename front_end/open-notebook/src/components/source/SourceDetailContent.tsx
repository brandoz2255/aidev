...
export function SourceDetailContent({
  ...
  // Citation deep-link: scroll-to + highlight the EXACT cited passage. `claim` = the
  // answer sentence/quote (primary target); `highlight` = the chunk's start (fallback).
  const searchParams = useSearchParams()
  const highlightTarget = searchParams?.get('highlight') || ''
  const claimTarget = searchParams?.get('claim') || ''
  const contentRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    if (loading || !source || (!highlightTarget && !claimTarget)) return
    let cancelled = false
    let attempts = 0
    const timers: number[] = []

    const clearMarks = (root: HTMLElement) => {
      root.querySelectorAll('mark[data-cite]').forEach((m) => {
    ...
    // Wrap a raw-text range [start,end) across text nodes in <mark>; return first mark.
    const markRange = (
      segs: { node: Text; start: number; len: number }[],
      start: number,
      end: number
    ): HTMLElement | null => {
      ...
      for (const s of segs) {
        ...
        if (segEnd <= start || s.start >= end) continue
        const a = Math.max(0, start - s.start)
        const b = Math.min(s.len, end - s.start)
        if (a >= b) continue
        try {
          const range = document.createRange()
          range.setStart(s.node, a)
          range.setEnd(s.node, b)
          const mark = document.createElement('mark')
          mark.setAttribute('data-cite', '1')
          range.surroundContents(mark)
          if (!first) first = mark
        } catch { /* segment boundary — skip */ }
      }
      return first
    }

    const findAndMark = (root: HTMLElement): HTMLElement | null => {
      clearMarks(root)
      // Build raw text + a whitespace-collapsed/lowercased normalized index, plus a
      // map normalized-index → raw-index (so we can match across paragraphs, where the
      // chunk text has "\n\n" the DOM doesn't, and tolerate whitespace differences).
      const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT)
      const segs: { node: Text; start: number; len: number }[] = []
      let raw = ''
      for (let n = walker.nextNode() as Text | null; n; n = walker.nextNode() as Text | null) {
        const v = n.nodeValue || ''
        segs.push({ node: n, start: raw.length, len: v.length })
        raw += v
      }
      if (!raw) return null
      // Normalize: collapse whitespace + drop quote chars (the answer often wraps a
      // quote the source text doesn't have) so they don't break the match.
      const QUOTES = /[“”"'‘’`]/
      let norm = ''
      ...
      for (let i = 0; i < raw.length; i++) {
        const c = raw[i]
        if (/\s/.test(c)) {
          if (norm.length && norm[norm.length - 1] !== ' ') { norm += ' '; normToRaw.push(i) }
        } else if (QUOTES.test(c)) {
          /* skip quote chars */
        } else {
          norm += c.toLowerCase(); normToRaw.push(i)
        }
      }
      const normalize = (s: string) =>
        s.toLowerCase().replace(/[“”"'‘’`]/g, '').replace(/\s+/g, ' ').trim()
      const tryFind = (q: string): HTMLElement | null => {
        const qn = normalize(q)
        if (qn.length < 8) return null
        const p = norm.indexOf(qn)
        if (p < 0) return null
        const rawStart = normToRaw[p]
        const rawEnd = normToRaw[p + qn.length - 1] + 1
        return markRange(segs, rawStart, rawEnd)
      }

      // 1) The claim, trimming framing words off the FRONT (e.g. "Here's a quote: …")
      //    until the remaining tail (the actual quote/fact) matches the source.
      if (claimTarget) {
        const words = normalize(claimTarget).split(' ')
        for (let s = 0; s <= words.length - 6 && s <= 14; s++) {
          const m = tryFind(words.slice(s).join(' '))
          if (m) return m
        }
      }
      // 2) Fallback: the chunk's start locator (full, then shorter prefixes).
      if (highlightTarget) {
        const loc = normalize(highlightTarget)
        for (const len of [loc.length, 60, 44, 28]) {
          if (len > loc.length) continue
          const m = tryFind(loc.slice(0, len))
          if (m) return m
        }
      }
      return null
    ...
    // Re-scroll a few times: the dialog open-animation / late layout can eat a single
    // scrollIntoView fired too early.
    const scrollToMark = (mark: HTMLElement) => {
      for (const d of [0, 160, 360]) {
        const t = window.setTimeout(() => {
          if (!cancelled) mark.scrollIntoView({ behavior: 'smooth', block: 'center' })
        }, d)
        timers.push(t)
      }
    }
    ...
    const tick = () => {
      ...
      const mark = root ? findAndMark(root) : null
      if (mark) { scrollToMark(mark); return }
      if (attempts < 20) {
  ...
  }, [loading, source, highlightTarget, claimTarget])
  ...
  const handleDeleteInsight = async (e?: React.MouseEvent) => {
    ...
    try {
      ...
      const deletedId = insightToDelete
      await insightsApi.delete(deletedId)
      toast.success(t('common.success'))
      setInsightToDelete(null)
      // If the deleted insight was open inline, return to the list.
      setSelectedInsight((cur) => (cur && cur.id === deletedId ? null : cur))
      await fetchInsights()
    } catch (err) {
          ...
          <TabsContent value="content" className="mt-6">
            <Card>
              ...
              <CardContent>
                ...
                <div ref={contentRef} className="prose prose-sm prose-neutral dark:prose-invert max-w-none prose-headings:font-semibold prose-a:text-blue-600 prose-code:bg-muted prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-p:mb-4 prose-p:leading-7 prose-li:mb-2">
                  <ReactMarkdown
          ...
          <TabsContent value="insights" className="mt-6">
            {selectedInsight ? (
              <Card>
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => setInsightToDelete(selectedInsight.id)}
                ...
                <CardContent>
                  <article className="prose prose-sm prose-neutral dark:prose-invert max-w-none prose-headings:font-semibold prose-headings:mt-6 prose-headings:mb-3 prose-headings:first:mt-0 prose-a:text-blue-600 prose-code:bg-muted prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-pre:bg-muted prose-p:mb-4 prose-p:leading-7 prose-li:mb-1.5 prose-ul:my-3 prose-ol:my-3">
                    <ReactMarkdown
                      remarkPlugins={[remarkGfm]}
                      components={{
                        table: ({ children }) => (
