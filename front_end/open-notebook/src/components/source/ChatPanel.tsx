...
interface ChatPanelProps {
  ...
  notebookId?: string
  // When provided, source citations open inline (parent-controlled) instead of a modal.
  onOpenSource?: (sourceId: string, highlight?: string, claim?: string) => void
  // Notebook object + live source count → drives the chat overview card (empty state).
...
export function ChatPanel({
  ...
  const handleReferenceClick = (type: string, id: string, locator?: string, claim?: string) => {
    ...
    // Source citations open inline in the notebook's left column (parent-controlled);
    // notes/insights still use the overlay modal.
    if (modalType === 'source' && onOpenSource) {
      onOpenSource(id, locator, claim)
      return
    }

    try {
      openModal(modalType, id, locator, claim)
...
function AIMessageContent({
  ...
  onReferenceClick: (type: string, id: string, locator?: string, claim?: string) => void
}) {
  // Convert [cite:N] / [source:uuid] markers → numbered chips (by distinct source,
  // deduped/grouped) + per-chip occurrences (source, locator, claim). Memoised.
  const cites = useMemo(() => citations || [], [citations])
  const { markdown: markdownWithCompactRefs, occurrences } = useMemo(
    () => convertCiteMarkers(content, cites),
    [content, cites]
  )
  // Chip link component: hover preview + click → open source at the cited passage.
  const LinkComponent = createCiteLinkComponent(onReferenceClick, occurrences)

  return (
    <div className="prose prose-sm prose-neutral dark:prose-invert max-w-none break-words prose-headings:font-semibold prose-a:text-blue-600 prose-a:break-all prose-code:bg-muted prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-p:mb-4 prose-p:leading-7 prose-li:mb-2 prose-pre:max-w-full prose-pre:overflow-x-auto [&_pre>code]:whitespace-pre-wrap [&_pre>code]:break-words">
      <ReactMarkdown
