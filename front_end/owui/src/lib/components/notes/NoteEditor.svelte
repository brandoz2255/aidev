	...
	<Pane defaultSize={70} minSize={30} class="h-full flex flex-col w-full relative">
					...
					<div class="shrink-0 w-full flex justify-between items-center px-3.5 mb-1.5">
						<div class="w-full min-w-0 flex items-center">
							...
							<div class="flex items-center gap-0.5 shrink-0 translate-x-1">
								...
								{#if note?.write_access}
									<button
										class="shrink-0 bg-gray-50 hover:bg-gray-100 text-black dark:bg-gray-850 dark:hover:bg-gray-800 dark:text-white transition px-2.5 py-1 rounded-lg flex gap-1.5 items-center text-sm"
										on:click={() => {
		...
		<div class="absolute z-50 bottom-0 right-0 p-3.5 flex select-none">
			<div class="flex flex-col gap-2 justify-end">
				...
				{:else}
					<div
						class="cursor-pointer flex gap-0.5 rounded-lg border border-gray-50 dark:border-gray-850/30 dark:bg-gray-850 transition shadow-xl"
					>
						<Tooltip content={$i18n.t('AI')} placement="top">
							{#if editing}
								<button
									class="p-2 flex justify-center items-center hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg transition shrink-0"
									on:click={() => {
										stopResponseHandler();
									}}
									type="button"
							...
							{:else}
								...
								>
									<div
										class="cursor-pointer p-2.5 flex rounded-lg border border-gray-50 bg-white dark:border-none dark:bg-gray-850 hover:bg-gray-50 dark:hover:bg-gray-800 transition shadow-xl"
									>
										<SparklesSolid />
									</div>
								</AiMenu>
