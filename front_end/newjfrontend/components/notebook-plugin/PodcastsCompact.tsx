...
export default function PodcastsCompact() {
  ...
  const loadPodcasts = async () => {
    ...
    try {
      ...
      setPodcasts((resp || []).map((p: any) => ({
        id: p.id,
        title: p.title || 'Untitled',
        status: p.status || 'pending',
        created: p.created_at || p.created,
        audio_url: p.audio_url || (p.audio_path && selectedNotebookId ? OpenNotebookAPI.podcasts.getAudioUrl(selectedNotebookId, p.id) : undefined),
      })))
    } catch {
      setPodcasts([])
    } finally {
  ...
  const handleGenerate = async () => {
    ...
    try {
      await OpenNotebookAPI.podcasts.generate({ notebook_id: selectedNotebookId, title: 'Podcast' })
      await loadPodcasts()
    } catch (err) {
  ...
  const handlePlay = (podcast: Podcast) => {
    ...
    const url = podcast.audio_url || (selectedNotebookId ? OpenNotebookAPI.podcasts.getAudioUrl(selectedNotebookId, podcast.id) : '')
