...
export default function SearchCompact() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const mapped: SearchResult[] = (resp || []).map((r: any) => ({
        id: r.source_id || r.note_id || String(Math.random()),
        type: r.kind === 'note' ? 'note' : 'source',
        title: r.title || r.notebook_title || 'Untitled',
        snippet: r.snippet || '',
