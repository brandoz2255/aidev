        ...
        {!sessionId || !isContainerRunning ? (
          ...
          <div className="flex gap-2">
            <Input
              ...
              className="flex-1 bg-muted border-border text-foreground placeholder:text-muted-foreground focus:border-purple-500"
            />
            <Button
      ...
      <Card className="bg-muted/50 border-border p-4">
        ...
        {hasOutput ? (
          <div className="space-y-2">
            {result.stdout && (
              <div>
                <div className="text-xs text-muted-foreground/60 mb-1">STDOUT:</div>
                <pre className="bg-background border border-border rounded p-3 text-sm text-green-400 font-mono overflow-x-auto whitespace-pre-wrap break-words">
                  {result.stdout}
                </pre>
              </div>
            )}
            {result.stderr && (
              <div>
                <div className="text-xs text-muted-foreground/60 mb-1">STDERR:</div>
                <pre className="bg-background border border-red-900/50 rounded p-3 text-sm text-red-400 font-mono overflow-x-auto whitespace-pre-wrap break-words">
                  {result.stderr}
                </pre>
              </div>
            )}
          ...
          <div className="bg-background border border-border rounded p-3">
            <p className="text-sm text-muted-foreground/60 italic">(no output)</p>
          </div>
        )}
      </Card>
    </motion.div>
