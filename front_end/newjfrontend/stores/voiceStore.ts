...
export const useVoiceStore = create<VoiceState>((set, get) => ({
  ...
  // Fetch all voices (user + built-in) -- silently handles missing endpoints
  fetchVoices: async () => {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          set({ isLoading: false, serviceAvailable: false })
          return
        }
  ...
  // Fetch all RVC character voices -- silently handles missing endpoints
