...
export default defineToolPlugin({
    ...
    description: "Web search via Harvis's DDG-backed proxy. Use when you need to verify a " +
        "fact, look up current information, or are uncertain about an answer.",
    tools: (tool) => [
        tool({
            name: "web_search",
            label: "Web Search",
            description: "Search the public web. Returns ranked results with titles, URLs, and " +
            ...
            async execute({ query, count = 5 }, _config, context) {
                ...
                if (!token) {
                    return {
                        error: "missing_token",
                        message: "OPENCLAW_GATEWAY_TOKEN env var is not set inside the OpenClaw " +
                ...
                try {
                    ...
                    return {
                        error: "fetch_failed",
                        message: `Network error reaching ${url}: ${err.message}`,
                        results: [],
                    };
                }
                if (!response.ok) {
                ...
                try {
                    ...
                    return {
                        error: "invalid_json",
                        message: `Proxy returned non-JSON: ${err.message}`,
                        results: [],
                    };
                }
                return {
