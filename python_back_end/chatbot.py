...
def extract_tab_command(message):
    ...
    if re.search(
        r"(?:open|create|launch|abre|crea)\s+(?:a\s+)?(?:blank|empty|new)\s+tab",
        message_lower,
    ):
...
def extract_search_query(message):
    ...
    for pattern in search_patterns:
        ...
        if match:
            ...
            query = re.sub(
                r"^(?:please|can you|could you|would you|will you|i want|i need|por favor|puedes|podrías)\s+",
                "",
                query,
            )
    ...
    try:
        ...
        if is_browser_command(message):
            ...
            try:
                ...
                return history, generate_speech(
                    response,
                    load_tts_model(),
                    audio_prompt,
                    exaggeration,
                ...
                error_msg = (
                    "¡Ay! Had trouble with that browser action. ¿Intentamos de nuevo?"
                )
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    with gr.Row():
        with gr.Column(scale=3):
            chatbot = gr.Chatbot(
                height=400, label="Conversation with Jarves", type="messages"
            )
        with gr.Column(scale=1):
            ...
            refresh.click(
                update_models, outputs=[model_selector, model_selector, model_status]
            )
            ...
            with gr.Accordion("🎛️ Advanced Settings", open=False):
                ...
                exaggeration = gr.Slider(
                    0.25, 2, value=0.5, step=0.05, label="Voice Expression"
                )
                temperature = gr.Slider(
                    0.05, 5, value=0.8, step=0.05, label="Creativity"
                )
                cfg_weight = gr.Slider(
                    0.0, 1, value=0.5, step=0.05, label="Speaking Pace"
                )
    ...
    with gr.Row():
        with gr.Column():
            ...
            voice_input = gr.Audio(
                sources=["microphone"], type="filepath", label="Record Your Message"
            )
        with gr.Column():
            gr.Markdown("### ⌨️ Escríbeme")
            msg = gr.Textbox(
                placeholder="Type here or use voice... Escribe aquí o usa tu voz...",
