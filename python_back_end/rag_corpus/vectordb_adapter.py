    ...
    async def initialize(self, embedding_dimension: int = None, recreate_if_mismatch: bool = False) -> None:
        ...
        async with self.db_pool.acquire() as conn:
            try:
                ...
                # Check if table exists and get current dimension/type
                # We fetch both atttypmod (dimension) and typname (type)
                ...
                existing_dim = row['atttypmod'] if row else None
                ...
                if "does not exist" in str(e):
                    ...
                    """)
                    self._initialized = True
                    logger.info(f"Created fresh vector table: {self.table_name}")
                else:
