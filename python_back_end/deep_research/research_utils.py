...
def strip_thinking(text):
    ...
    cleaned = _re.sub(r"<think(?:ing)?>.*?</think(?:ing)?>", "", text,
                      flags=_re.DOTALL | _re.IGNORECASE)
    # Drop an unclosed leading <think> ... (model truncated before </think>).
    cleaned = _re.sub(r"<think(?:ing)?>.*$", "", cleaned,
                      flags=_re.DOTALL | _re.IGNORECASE)
    # Strip any stray tags that remain.
