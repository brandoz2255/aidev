...
class DeepResearcher:
    ...
    async def _llm(self, messages: List[Dict], temperature: float = 0.3,
        ...
        """Call local Ollama directly (no cloud-first fallback) and strip thinking.

        research_agent.async_make_ollama_request pings a cloud endpoint first and
        only falls back to local — that round-trip per call dominated wall-clock in
        the first E2E. Deep research is local-only, so hit ollama directly.
        """
        import os
        import httpx
        ...
        async with httpx.AsyncClient(timeout=timeout) as cli:
            r = await cli.post(f"{base}/api/chat", json=payload)
        r.raise_for_status()
    ...
    async def _search(self, query: str) -> List[Dict]:
        """Run a search query using Harvis's web search agent."""
        try:
            from research.web_search import WebSearchAgent

            agent = WebSearchAgent(max_results=10)
            results = await asyncio.to_thread(agent.search_web, query)
            if results:
                logger.info(f"Research search: harvis-web returned {len(results)} results")
                if "harvis-web" not in self.providers_used:
                    self.providers_used.append("harvis-web")
                # Normalize to the {url, title, snippet} shape the loop expects.
                return [
    ...
    async def _fetch_and_extract(self, url: str, question: str,
        ...
        if not getattr(doc, "success", False) or not getattr(doc, "text", ""):
            return None

        # Normalize to the page dict the rest of this method expects.
        page = {"content": doc.text, "title": doc.title or "", "og_image": ""}
    ...
    def _emit(self, **kwargs):
        ...
        if self._progress:
            try:
                kwargs.setdefault("total_sources", len(self.urls_fetched))
                # Carry accumulated source URLs so the UI can render sources-by-domain
                # (the SSE poll only forwards the latest event, so include the full set).
                kwargs["sources"] = [{"url": u} for u in list(self.urls_fetched)]
                kwargs["queries"] = list(self.queries_used)
