...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Pre-warm Docker images for faster container creation
        try:
            import docker
            docker_client = docker.from_env()
            vibecoding_image = "vibecoding-optimized:latest"
            docker_client.images.pull(vibecoding_image)
            logger.info(f"✅ Pre-pulled {vibecoding_image}")
        except Exception as e:
            logger.warning(f"⚠️ Image warm failed: {e}")
