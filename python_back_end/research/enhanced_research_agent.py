...
from .pipeline.research_agent import (
    ResearchAgent as AdvancedResearchAgent,
    ResearchConfig,
)
...
class EnhancedResearchAgent:
    ...
    def __init__(
        ...
        model: str = None,  # REQUIRED - no default, must be passed from caller
    ...
    def _create_simple_response(
        self, analysis: str, sources: List[Dict], model: str
    ) -> Dict[str, Any]:
...
Comparative analysis:"""
        ...
        try:
            if enable_streaming:
                ...
                return (
                    result.response
                    if result.success
                    else f"Research failed: {result.error}"
                )
    ...
    async def advanced_compare(
        self, topics: List[str], context: str = None, model: str = None
    ) -> str:
        ...
        try:
            result = await self.comparative_analyzer.compare(
                topics, comparison_context=context
            )
            return result.response
        except Exception as e:
