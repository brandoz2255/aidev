...
class WebSearchAgent:
    ...
    def search_web(self, query: str, num_results: int = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Original query: '{query}' -> Improved query: '{improved_query}'"
            )

            logger.info(
                f"Starting DDGS search for original query: '{query}', improved query: '{improved_query}' with max_results: {max_results}"
            )
            ...
            for i, result in enumerate(search_results[:3]):
                logger.info(
                    f"Raw result {i + 1}: Title='{result.get('title', 'N/A')}', URL='{result.get('href', 'N/A')}'"
                )
            ...
            for result_data in scored_results[:max_results]:
                ...
                logger.debug(
                    f"Added result (score: {result_data['score']:.2f}): {formatted_result['title'][:50]}..."
                )

            logger.info(
                f"Found {len(formatted_results)} relevant search results for query: '{query}'"
            )

            # If we still have no results, lower the threshold even more
            if not formatted_results and scored_results:
                logger.warning(
                    "No results passed initial filtering, including lower-scored results"
                )
                for result_data in scored_results[:max_results]:
                    result = result_data["result"]
                    formatted_result = {
            ...
            if "rate limit" in str(e).lower():
                logger.error(
                    "Rate limited by DuckDuckGo. Consider adding delays between requests."
                )
    ...
    def extract_content_from_urls(self, urls: List[str]) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Successfully extracted content from {len(successful_results)} out of {len(urls)} URLs"
        )

        return results

    def search_and_extract(
        self, query: str, extract_content: bool = True
    ) -> Dict[str, Any]:
        """
    ...
    def _score_and_filter_results(
        ...
        logger.info(
            f"Scored {len(scored_results)} relevant results from {len(search_results)} total"
        )
        return scored_results

    def _calculate_relevance_score(
        self, result: Dict, query_words: set, _: str
    ) -> float:
        """
        ...
        try:
            ...
            for video in video_results:
                ...
                formatted_video = {
                    ...
                    "description": video.get("description", video.get("body", ""))[
                        :200
                    ],
            ...
            logger.info(
                f"Found {len(formatted_videos)} YouTube videos for query: '{query}'"
            )
    ...
    def search_with_videos(
        self, query: str, max_web_results: int = 5, max_video_results: int = 4
    ) -> Dict[str, Any]:
        """
    ...
    def search_and_extract_with_videos(
        self,
        query: str,
        ...
        if extract_content and search_results:
            ...
            urls = [
                ...
                if r.get("url")
                and "youtube.com" not in r["url"]
                and "youtu.be" not in r["url"]
            ]
    ...
    def fetch_video_transcripts(
        self, videos: List[Dict[str, Any]], max_videos: int = 3, timeout_s: int = 15
    ) -> List[Dict[str, Any]]:
        """
        ...
        def fetch_single_transcript(video: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                transcript_url = (
                    url if url else f"https://youtube.com/watch?v={video_id}"
                )
                ...
                if transcript_data.get("success") and transcript_data.get("text"):
                    ...
                    logger.info(
                        f"Successfully fetched transcript for: {video.get('title', 'Unknown')[:50]}"
                    )
                else:
                    video["transcript"] = ""
                    video["hasTranscript"] = False
                    logger.debug(
                        f"No transcript available for: {video.get('title', 'Unknown')[:50]}"
                    )
        ...
        with ThreadPoolExecutor(max_workers=min(3, len(videos_to_process))) as executor:
            ...
            try:
                ...
                logger.warning(
                    "Transcript fetching timed out, returning partial results"
                )

        # Count successful transcripts
        success_count = sum(
            1 for v in videos_to_process if v.get("hasTranscript", False)
        )
        logger.info(
            f"Fetched {success_count}/{len(videos_to_process)} video transcripts"
        )
...
class TavilySearchAgent:
    ...
    def search_web(self, query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Found {len(results)} Tavily search results for query: {query}"
            )
            return results
