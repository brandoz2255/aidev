...
class OllamaClient:
    ...
    def __init__(
        ...
        self.model = model  # Use passed model
        self.fallback_models = []  # NO fallback - fail if model doesn't work
            ...
            try:
                ...
                if not self._validate_response(response):
                    raise GenerationError(
                        f"Empty or invalid response from {attempt_model}"
                    )
                ...
                last_error = str(e)
                logger.warning(
                    f"Request failed with model {attempt_model}: {last_error}"
                )
                continue
    ...
    async def generate(
        ...
        **kwargs,
        ...
        return await self._make_request_with_fallback(
            prompt=prompt, model=model, **generation_kwargs
        )
    ...
    async def batch_generate(
        ...
        **kwargs,
        ...
        responses = await asyncio.gather(
            *[bounded_generate(prompt) for prompt in prompts], return_exceptions=True
        )
        ...
        logger.info(
            f"Batch completed in {total_time:.2f}s: {successful}/{len(prompts)} successful"
        )
...
async def quick_generate(
    prompt: str, model: str = "mistral", max_retries: int = 2
) -> str:
...
async def batch_quick_generate(
    prompts: List[str], model: str = "mistral", max_concurrent: int = 3
) -> List[str]:
